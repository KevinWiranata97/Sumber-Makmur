    ...
    try {
      ...
      items.forEach((item, rowIndex) => {
        ...
        row.eachCell((cell, colIndex) => {
          ...
          cell.font = { size: 10, name: "Courier New" };
        });
      });
      ...
      worksheet.getCell(`A${summaryStartRow}`).font = { size: 10, name: "Courier New", underline: "true" };
      ...
      worksheet.getCell(`G${summaryStartRowTotal}`).font = { size: 10, name: "Courier New" };
      worksheet.getCell(`I${summaryStartRowTotal}`).value = `Rp.${invoiceData.subTotal.toLocaleString('id-ID')}`;
      worksheet.getCell(`I${summaryStartRowTotal}`).font = { size: 10, name: "Courier New" };
      ...
      worksheet.getCell(`G${summaryStartRowTotal + 1}`).font = { size: 10, name: "Courier New" };
      ...
      worksheet.getCell(`I${summaryStartRowTotal + 1}`).font = { size: 10, name: "Courier New" };
      ...
      worksheet.getCell(`G${summaryStartRowTotal + 2}`).font = { size: 10, name: "Courier New" };
      ...
      worksheet.getCell(`I${summaryStartRowTotal + 2}`).font = { size: 10, name: "Courier New" };
      // Add border to PPN 11% cells
      ...
      worksheet.getCell(`G${summaryStartRowTotal + 3}`).font = { size: 9, name: "Courier New" };
      ...
      paymentCell.font = { size: 9, name: "Book Antiqua" };
      ...
      worksheet.getCell(`A${tandaTerimaRow}`).alignment = { horizontal: 'left' }
      worksheet.getCell(`A${tandaTerimaRow}`).font = { size: 9, name: "Courier New" };
      // Add 'Hormat Kami' text on the right
      worksheet.getCell(`I${tandaTerimaRow}`).value = 'Hormat Kami,';
      worksheet.getCell(`I${tandaTerimaRow}`).font = { size: 9, name: "Courier New" };
      ...
      worksheet.getCell(`A${signatureRow}`).font = { size: 9, name: "Courier New" };
      ...
      worksheet.getCell(`I${signatureRow}`).font = { size: 9, name: "Courier New" };
      ...
      workbook.xlsx.writeFile(filePath).then(() => {
        resolve(); // Resolve the promise after the file is written
      }).catch((err) => {
        reject(err); // Reject the promise if there is an error
      });
    } catch (error) {
      reject(error); // Reject the promise if there is an unexpected error
    }
  });
...
function generateSuratJalanExcel(invoiceData, filePath) {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      worksheet.getCell(`G${tandaTerimaRow}`).font = { size: 9, name: "Courier New" };
      ...
      worksheet.getCell(`G${signatureRow}`).font = { size: 9, name: "Courier New" };
...
function generateInvoiceExcelBuy(invoiceData, filePath) {
  return new Promise((resolve, reject) => {
    try {
      const workbook = new ExcelJS.Workbook();
      const worksheet = workbook.addWorksheet('Sheet1 (7)');
      ...
      worksheet.mergeCells('A2:E2');
      worksheet.getCell('A2').value = 'Jakarta Pusat - 10710';
      worksheet.getCell('A2').font = { name: "Courier New", size: 12 };
      worksheet.getCell('A2').alignment = { horizontal: 'center' };

      worksheet.mergeCells('A4:E4');
      worksheet.getCell('A4').value = `SUPPLIER  :${invoiceData.customer.name}`;
      worksheet.getCell('A4').font = { name: "Courier New", size: 10 };
      worksheet.getCell('A4').alignment = { horizontal: 'left' };

      worksheet.mergeCells('A5:E5');
      worksheet.getCell('A5').value = `${invoiceData.customer.address }`
      worksheet.getCell('A5').font = { name: "Courier New", size: 10 };
      ...
      worksheet.mergeCells('A7:E7');
      worksheet.getCell('A7').value = `PEMBELIAN BARANG`
      worksheet.getCell('A7').font = { name: "Courier New", size: 14 };
      worksheet.getCell('A7').alignment = { horizontal: 'right' };
      ...
      worksheet.mergeCells('G3:I3');
      worksheet.getCell('G3').value = `No. Bukti : ${invoiceData.proofNumber}`;
      worksheet.getCell('G3').alignment = { horizontal: 'left' };
      worksheet.getCell('G3').font = { name: "Courier New", size: 10 };

      worksheet.mergeCells('G4:I4');
      worksheet.getCell('G4').value = `Tanggal   : ${invoiceData.transaction_date}`;
      worksheet.getCell('G4').alignment = { horizontal: 'left' };
      worksheet.getCell('G4').font = { name: "Courier New", size: 10 };

      worksheet.mergeCells('G5:I6');
      worksheet.getCell('G5').value = `Tgl Tempo : ${invoiceData.transaction_due_date}`;
      worksheet.getCell('G5').alignment = { horizontal: 'left', wrapText: "true", vertical: "middle" };
      worksheet.getCell('G5').font = { name: "Courier New", size: 10 };
      ...
      worksheet.addRow(['No', 'Qty', '', 'PART NUMBER', 'DESCRIPTION', '', 'COST', 'AMOUNT']);
      ...
      // // Set values for the merged cells
      worksheet.getCell('F8').value = 'COST'; // Use a line break to separate "UNIT" and "PRICE"
      // worksheet.getCell('F9').value = 'PRICE'; // Use a line break to separate "UNIT" and "PRICE"
      worksheet.getCell('F9').alignment = { horizontal: "center", vertical: "middle" }
      worksheet.getCell('H8').value = 'AMOUNT'; // Set the value for "AMOUNT"
      ...
      // PPN 11%
      worksheet.mergeCells(`G${summaryStartRowTotal + 1}:H${summaryStartRowTotal + 1}`);
      worksheet.getCell(`G${summaryStartRowTotal + 1}`).value = `PPN ${invoiceData.tax_ppn}%`;
      worksheet.getCell(`G${summaryStartRowTotal + 1}`).font = { size: 10, name: "Courier New" };
      worksheet.getCell(`G${summaryStartRowTotal + 1}`).alignment = { horizontal: 'right' };
      worksheet.getCell(`I${summaryStartRowTotal + 1}`).value = `Rp.${invoiceData.totalPpn.toLocaleString('id-ID')}`;
      worksheet.getCell(`I${summaryStartRowTotal + 1}`).font = { size: 10, name: "Courier New" };
      // Add border to PPN 11% cells
      worksheet.getCell(`G${summaryStartRowTotal + 1}`).border = {
      ...
      // Grand Total
      worksheet.mergeCells(`G${summaryStartRowTotal + 2}:H${summaryStartRowTotal + 2}`);
      worksheet.getCell(`G${summaryStartRowTotal + 2}`).value = 'GRAND TOTAL';
      worksheet.getCell(`G${summaryStartRowTotal + 2}`).alignment = { horizontal: 'right', vertical: 'middle' };
      worksheet.getCell(`G${summaryStartRowTotal + 2}`).font = { size: 9, name: "Courier New" };
      worksheet.getCell(`I${summaryStartRowTotal + 2}`).value = `Rp.${invoiceData.grandTotal.toLocaleString('id-ID')}`;
      worksheet.getCell(`I${summaryStartRowTotal + 2}`).font = { size: 12, name: "Courier New" };
      // Add border to Grand Total cells (with thicker bottom border)
      worksheet.getCell(`G${summaryStartRowTotal + 2}`).border = {
...
module.exports = { generateInvoiceExcel, generateInvoiceExcelNonPpn, generateSuratJalanExcel, generateInvoiceExcelBuy }