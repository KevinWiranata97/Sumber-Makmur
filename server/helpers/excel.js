...
worksheet.columns = [
  ...
  worksheet.getCell('A7').font = { name: "Arial", size: 14 };
  ...
  worksheet.getCell('G4').font = { name: "Arial", size: 10 };
  ...
  worksheet.getCell('G7').font = { name: "Arial", size: 10 };
  ...
  headerRow.font = { size: 10, name: "Arial" };
  ...
  let subtotal = 0;
  items.forEach(item => {
    const amountString = item[item.length - 1]; // Get the 'AMOUNT' column value
    const amount = parseFloat(amountString.replace(/,/g, '')); // Convert string to number (remove commas)
    subtotal += amount;
  });
  ...
  items.forEach((item, rowIndex) => {
    ...
    row.eachCell((cell, colIndex) => {
      ...
      // Apply the border and alignment to each cell
      cell.border = border;
  ...
  worksheet.getCell(`A${summaryStartRow }`).font = { bold: true, size: 10, name:"Arial" ,underline:"true"};
  ...
  worksheet.getCell(`G${summaryStartRowTotal}`).font = { bold: true, size: 10, name:"Arial" };
  worksheet.getCell(`I${summaryStartRowTotal}`).value = `Rp. ${invoiceData.subTotal.toLocaleString('id-ID')}`;
  worksheet.getCell(`I${summaryStartRowTotal}`).font = { bold: true, size: 10, name:"Arial" };
  ...
  worksheet.getCell(`G${summaryStartRowTotal + 1}`).font = { bold: true, size: 10, name:"Arial" };
  ...
  worksheet.getCell(`I${summaryStartRowTotal + 1}`).font = { bold: true, size: 10, name:"Arial" };
  ...
  worksheet.getCell(`G${summaryStartRowTotal + 2}`).font = { bold: true, size: 10, name:"Arial" };
  ...
  worksheet.getCell(`I${summaryStartRowTotal + 2}`).font = { bold: true, size: 10, name:"Arial" };
  // Add border to PPN 11% cells
  ...
  worksheet.getCell(`G${summaryStartRowTotal + 3}`).alignment = { horizontal: 'right', vertical: 'top' };
  worksheet.getCell(`G${summaryStartRowTotal + 3}`).font = { bold: true, size: 9, name:"Arial" };
  worksheet.getCell(`I${summaryStartRowTotal + 3}`).value = `Rp. ${invoiceData.grandTotal.toLocaleString('id-ID')}`;
  worksheet.getCell(`I${summaryStartRowTotal + 3}`).font = { size: 13, name:"Arial" };
  ...
  worksheet.getCell(`I${tandaTerimaRow}`).alignment = { horizontal: 'right' };
  ...
  worksheet.getCell(`I${signatureRow}`).alignment = { horizontal: 'right' };
...
// Header Information (starting from A1)
