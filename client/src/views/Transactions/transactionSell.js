...
const MyModal = ({ showModal, handleClose, data, fungsi }) => {
  ...
  const handleChange = (e) => {
    ...
    const validatedValue =
      name === "transaction_discount" ? Math.min(numericValue, 100) : value;
  ...
  const handleCustomerSelectChange = (selectedOption) => {
    ...
    const selectedCustomer = customers.find(
      (customer) => customer.id === parseInt(selectedOption.id)
    );
      ...
      if (response) {
        ...
        while (attempt < maxRetries) {
          try {
            ...
            console.warn(
              `File not ready yet. Retrying in ${retryDelay / 1000} seconds...`
            );
          }
        ...
        link.href = fileUrl;
        link.download = isSuratJalanChecked
          ? "surat_jalan.xlsx"
          : "invoice.xlsx"; // Change file name based on the checkbox
    ...
    return rows.reduce(
      (totals, row) => {
        ...
        totals.total_discount =
          totals.totalCost * (formData.transaction_discount / 100);

        totals.total_after_disc =
          totals.totalCost -
          totals.totalCost * (formData.transaction_discount / 100);
        ...
        totals.total_discountFormatted = `Rp. ${totals.total_discount.toLocaleString(
          "id-ID"
        )}`;
            ...
            <div className="form-section row g-3">
                ...
                {!data ? (
                  <Select
                    value={
                      customers.find(
                        (customer) =>
                          customer.id === formData.transaction_customer_id
                      ) || null
              ...
              <div className="col-md-4 mt-2">
                ...
                <Select
                  value={
                    expeditions.find(
                      (expedition) => expedition.id === customerExpedition
                    ) || null
                  } // Match selected value
              ...
              <div className="col-md-4 mt-2">
                ...
                <input
                  type="text"
                  className="form-control"
                  name="transaction_PO_num"
                  onChange={handleChange}
                  value={formData.transaction_PO_num}
                />
              </div>
              <div className="col-md-4 mt-2">
                <div>
                  ...
                  <input
                    ...
                    id="noInvoice"
                    readOnly
                    value={data ? data.transaction_surat_jalan : ""}
                  />
                </div>
              </div>
                              ...
                              <input
                                ...
                                value={
                                  data
                                    ...
                                    : totals.total_discountFormatted
                                }
                                readOnly
                              />
                            </div>
  ...
  const fetchTransactions = async (value) => {
    ...
    try {
      ...
      const response = await axios({
        ...
        }/transactions?type=sell&search=${searchTerm}&limit=${pageSize}&page=${
          page + 1
        }`,
        headers: {
          authorization: localStorage.getItem("authorization"),
